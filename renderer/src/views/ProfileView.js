...
const ProfileView = () => {
    ...
    const [accuracy, setAccuracy] = react_1.default.useState(null);
    const [eloResult, setEloResult] = react_1.default.useState(null);
    ...
    const handleCalculate = async () => {
        ...
        setLoading(true);
        try {
            const res = await window.electronAPI.estimateElo({ accuracy: 60, blunder_rate: 0 });
            setAccuracy(60);
            setEloResult(res);
        }
        catch { /* ignore */ }
    ...
    return (<div className="flex flex-col gap-4 w-full py-4">
      ...
      <div className="bg-surface rounded-card border border-surface2 p-4 flex flex-col gap-3">
        ...
        {accuracy !== null && eloResult ? (<div className="flex flex-col gap-1">
          ...
          </div>) : (<p className="text-xs text-muted">Play a game to see your performance stats.</p>)}
        <button onClick={handleCalculate} disabled={loading || moveHistory.length < 2} className="w-full py-2 border border-surface2 rounded-lg text-xs font-sans text-muted
