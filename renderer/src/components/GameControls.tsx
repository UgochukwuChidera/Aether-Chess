...
interface ConfirmState {
  type: 'draw' | 'resign' | null;
}

export const GameControls: React.FC<Props> = ({ onFlip, onUndo, onDraw, onResign }) => {
  const { engineBusy, gameResult } = useGameStore();
  ...
  return (
    <>
      <div className="flex gap-2 w-full">
        {buttons.map(({ icon, label, action, disabled }) => (
          <button
            key={label}
            onClick={action}
            disabled={disabled}
          ...
          <div className="bg-surface border border-surface2 rounded-card p-5 w-72 shadow-2xl">
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 py-2 bg-accent text-bg rounded-lg font-sans font-semibold text-sm
              ...
              <button
                ...
                className="flex-1 py-2 border border-surface2 text-muted rounded-lg font-sans text-sm
