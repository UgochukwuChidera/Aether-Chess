...
export const TopBar: React.FC<Props> = ({ title = 'AETHER CHESS', onMenuClick, onSettingsClick }) => {
  ...
  return (
    <header
      className="drag-region fixed top-0 left-0 right-0 z-50 flex items-center h-16
    ...
    >
      ...
      <div className="no-drag flex items-center ml-2 gap-1">
        <button
          ...
          className="w-8 h-8 flex items-center justify-center rounded text-inactive
                     hover:text-on-surface hover:bg-surface2 transition-colors"
          aria-label="Minimize"
        >
        ...
        <button
          ...
          className="w-8 h-8 flex items-center justify-center rounded text-inactive
                     hover:text-error hover:bg-[#93000A] transition-colors"
          aria-label="Close"
        >
